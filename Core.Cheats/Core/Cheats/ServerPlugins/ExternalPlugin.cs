...
public class ExternalPlugin : IRestServerPlugin
{
	private static readonly JsonSerializer JsonSerializer = new JsonSerializer();

	private readonly CheatsParser.ExecuteExternalDelegate m_Executor;

	public HttpMethod HttpMethod => HttpMethod.Post;

	public string LocalPath => "/External";

	public ExternalPlugin(CheatsParser.ExecuteExternalDelegate executor)
	{
		m_Executor = executor;
	}
	...
	public async Task Handle(HttpListenerContext context, CancellationToken cancellationToken)
	{
		...
		ExecuteExternalInfo executeExternalInfo = JsonSerializer.Deserialize<ExecuteExternalInfo>(jsonReader);
		context.Response.ContentType = "text/plain";
		await using StreamWriter stream = new StreamWriter(context.Response.OutputStream);
		using JsonTextWriter writer = new JsonTextWriter(stream);
		try
		{
			await m_Executor(executeExternalInfo.ExternalName, executeExternalInfo.ExternalNameWithArgs);
		}
