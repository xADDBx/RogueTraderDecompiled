...
public static class ClassicNoise3D
{
	...
	public static float cnoise(float3 P)
	{
		...
		x3 = math.frac(x3);
		float4 threshold = (float4)0.5 - math.abs(x3) - math.abs(x4);
		float4 float9 = math.step(threshold, 0f);
		x3 -= float9 * (math.step(0f, x3) - 0.5f);
		x4 -= float9 * (math.step(0f, x4) - 0.5f);
		float4 x5 = float8 / 7f;
		float4 x6 = math.frac(math.floor(x5) / 7f) - 0.5f;
		x5 = math.frac(x5);
		float4 threshold2 = (float4)0.5f - math.abs(x5) - math.abs(x6);
		float4 float10 = math.step(threshold2, (float4)0.0);
		x5 -= float10 * (math.step(0f, x5) - 0.5f);
		x6 -= float10 * (math.step(0f, x6) - 0.5f);
		float3 float11 = new float3(x3.x, x4.x, threshold.x);
		float3 float12 = new float3(x3.y, x4.y, threshold.y);
		float3 float13 = new float3(x3.z, x4.z, threshold.z);
		float3 float14 = new float3(x3.w, x4.w, threshold.w);
		float3 float15 = new float3(x5.x, x6.x, threshold2.x);
		float3 float16 = new float3(x5.y, x6.y, threshold2.y);
		float3 float17 = new float3(x5.z, x6.z, threshold2.z);
		float3 float18 = new float3(x5.w, x6.w, threshold2.w);
		...
		float y2 = math.dot(float12, new float3(y.x, float2.y, float2.z));
		...
		float y3 = math.dot(float16, new float3(y.x, float2.y, y.z));
		float z2 = math.dot(float17, new float3(float2.x, y.y, y.z));
		float w2 = math.dot(float18, y);
		float3 float21 = fade(float2);
		float4 float22 = math.lerp(new float4(x7, y2, z, w), new float4(x8, y3, z2, w2), float21.z);
	...
	public static float pnoise(float3 P, float3 rep)
	{
		...
		x5 = math.frac(x5);
		float4 threshold2 = (float4)0.5f - math.abs(x5) - math.abs(x6);
		float4 float10 = math.step(threshold2, 0f);
		x5 -= float10 * (math.step(0f, x5) - 0.5f);
		x6 -= float10 * (math.step(0f, x6) - 0.5f);
		float3 float11 = new float3(x3.x, x4.x, threshold.x);
		float3 float12 = new float3(x3.y, x4.y, threshold.y);
