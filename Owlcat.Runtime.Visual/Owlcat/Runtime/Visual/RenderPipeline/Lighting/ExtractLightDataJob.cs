...
public struct ExtractLightDataJob : IJobFor
{
	...
	public void Execute(int index)
	{
		...
		uint x = PackData(lightDescriptor.LightFalloffType, lightDescriptor.SnapSpecularToInnerRadius, lightDescriptor.ShadowDataIndex, lightDescriptor.ShadowmaskChannel);
	...
	private uint PackData(LightFalloffType lightFalloffType, bool snapSpecularToInnerRadius, int shadowDataIndex, int shadowmaskChannel)
	{
		uint num = 0u;
		num |= ((lightFalloffType == LightFalloffType.Legacy) ? 1u : 0u);
		num |= (snapSpecularToInnerRadius ? 2u : 0u);
		if (shadowDataIndex > -1 && shadowDataIndex < 64)
		{
		...
		else if (shadowDataIndex >= 64)
		{
			Debug.LogError($"INVALID SHADOW DATA INDEX: {shadowDataIndex}");
		}
		num |= ((shadowmaskChannel > -1) ? 512u : 0u);
		if (shadowmaskChannel > -1)
		{
			num |= (uint)(shadowmaskChannel << 10);
		}
		return num;
	}
}
