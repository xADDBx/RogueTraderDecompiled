...
public static class PostProcessUtils
{
	...
	public static int ConfigureDithering(PostProcessData data, int index, Camera camera, Material material)
	{
		...
		material.SetVector(ShaderConstants._Dithering_Params, new Vector4((float)camera.pixelWidth / (float)texture2D.width, (float)camera.pixelHeight / (float)texture2D.height, value, value2));
		return index;
	}
	...
	public static void ConfigureFilmGrain(PostProcessData data, FilmGrain settings, Camera camera, Material material)
	{
		...
		float value = Random.value;
		float value2 = Random.value;
		Vector4 value3 = ((texture == null) ? Vector4.zero : new Vector4((float)camera.pixelWidth / (float)texture.width, (float)camera.pixelHeight / (float)texture.height, value, value2));
