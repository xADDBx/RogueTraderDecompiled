...
internal sealed class FogOfWarCullingSystem : IDisposable
{
	...
	{
		public readonly List<FogOfWarBlocker> StaticBlockers = new List<FogOfWarBlocker>();

		public readonly List<FogOfWarBlocker> DynamicBlockers = new List<FogOfWarBlocker>();
		...
		public void RegisterStaticBlocker(FogOfWarBlocker blocker)
		{
			blocker.m_CullingRegistryIndex = StaticBlockers.Count;
			StaticBlockers.Add(blocker);
			StaticBlockersChanged = true;
		}

		public void UnregisterStaticBlocker(FogOfWarBlocker blocker)
		{
			int cullingRegistryIndex = blocker.m_CullingRegistryIndex;
			int num = StaticBlockers.Count - 1;
			blocker.m_CullingRegistryIndex = -1;
			if (cullingRegistryIndex != num)
			{
				StaticBlockers[cullingRegistryIndex] = StaticBlockers[num];
				StaticBlockers[cullingRegistryIndex].m_CullingRegistryIndex = cullingRegistryIndex;
			}
			StaticBlockers.RemoveAt(num);
			StaticBlockersChanged = true;
		}

		public void RegisterDynamicBlocker(FogOfWarBlocker blocker)
		{
			blocker.m_CullingRegistryIndex = DynamicBlockers.Count;
			DynamicBlockers.Add(blocker);
		}

		public void UnregisterDynamicBlocker(FogOfWarBlocker blocker)
		{
			int cullingRegistryIndex = blocker.m_CullingRegistryIndex;
			...
			if (cullingRegistryIndex != num)
			{
				...
				DynamicBlockers[cullingRegistryIndex].m_CullingRegistryIndex = cullingRegistryIndex;
			}
	...
	private sealed class JobScene : IDisposable
	{
		...
		public unsafe void Update(GameScene gameScene, in NativeList<RevealerProperties> revealers)
		{
			if (gameScene.StaticBlockersChanged)
			{
				StaticBlockerSegments.Clear();
				foreach (FogOfWarBlocker staticBlocker in gameScene.StaticBlockers)
				{
					StaticBlockerSegments.AddRange(staticBlocker.m_CullingSegments.Ptr, staticBlocker.m_CullingSegments.Length);
				}
			}
			DynamicBlockerSegments.Clear();
			foreach (FogOfWarBlocker dynamicBlocker in gameScene.DynamicBlockers)
			{
				DynamicBlockerSegments.AddRange(dynamicBlocker.m_CullingSegments.Ptr, dynamicBlocker.m_CullingSegments.Length);
			}
			RevealerProperties.CopyFrom(revealers);
		...
		private unsafe void UpdateAllTargets(List<ITarget> targets)
		{
			...
			TargetProperties* ptr = (TargetProperties*)TargetProperties.GetUnsafePtr();
			bool* ptr2 = (bool*)TargetRevealedStates.GetUnsafePtr();
			bool* ptr3 = (bool*)TargetForceRevealStates.GetUnsafePtr();
		...
		{
			TargetProperties* unsafePtr = (TargetProperties*)TargetProperties.GetUnsafePtr();
			foreach (ITarget target in targets)
			{
				int registryIndex = target.RegistryIndex;
		...
		{
			bool* ptr = (bool*)TargetForceRevealStates.GetUnsafePtr();
			foreach (ITarget target in Targets)
			{
	...
	private void ScheduleJobs()
	{
		...
		if (m_GameScene.StaticBlockersChanged)
		{
			...
			dependsOn = jobData.Schedule(length3, 128);
		}
		else
		{
		...
		JobHandle job = jobData2.Schedule(length, 2, dependsOn);
		jobData = default(BuildBlockerPlanesJob);
		...
		JobHandle dependsOn2 = jobData.Schedule(length4, 128);
		...
		JobHandle job2 = jobData2.Schedule(length, 2, dependsOn2);
		...
		JobHandle value = jobData3.Schedule(m_JobScene.TargetProperties.Length, 32, JobHandle.CombineDependencies(job, job2));
