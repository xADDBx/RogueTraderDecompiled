...
public class PostProcessPass : ScriptableRenderPass
{
	...
	private RenderFunc<RcasPassData> m_RcasRenderFunc;
	...
	private void RenderFinalPostProcess(ref RenderingData renderingData)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<FinalPostProcessPassData>("Final Post Process Pass", out passData2);
	...
	private void RenderPostProcess(ref RenderingData renderingData)
	{
		...
		RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<PostProcessPassDataBase>("StopNaN and SRGBConversion", out passData2);
		try
		{
		...
		RenderGraphBuilder renderGraphBuilder2 = renderGraph.AddRenderPass<PostProcessPassDataBase>("Final Blit", out passData3);
		try
		{
	...
	{
		PostProcessPassDataBase passData2;
		RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<PostProcessPassDataBase>("Copy History", out passData2);
		try
		{
			passData2.Materials = m_Materials;
	...
	private void DoTemporalAntialiasing(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<TaaPassData>("TAA", out passData2);
		try
		{
			passData2.Materials = m_Materials;
			passData2.Source = renderGraphBuilder.ReadTexture(in source);
	...
	private void DoSubpixelMorphologicalAntialiasing(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<SmaaPassData>("SMAA", out passData2);
		passData2.Source = renderGraphBuilder.ReadTexture(in source);
	...
	private void DoGaussianDepthOfField(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<GaussianDofPassData>("Depth Of Field (Gaussian)", out passData2);
	...
	private void DoBokehDepthOfField(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<BokehDofPassData>("Depth Of Field (Bokeh)", out passData2);
	...
	private void DoMotionBlur(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using (RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<MotionBlurPassData>("Motion Blur", out passData2))
		{
			passData2.Source = renderGraphBuilder.ReadTexture(in source);
			passData2.Destination = renderGraphBuilder.WriteTexture(in destination);
			passData2.Materials = m_Materials;
	...
	private void DoRadialBlur(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		PostProcessPassDataBase passData2;
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<PostProcessPassDataBase>("Radial Blur", out passData2);
		passData2.Source = renderGraphBuilder.ReadTexture(in source);
		passData2.Destination = renderGraphBuilder.WriteTexture(in destination);
		passData2.Materials = m_Materials;
		renderGraphBuilder.SetRenderFunc(delegate(PostProcessPassDataBase passData, RenderGraphContext context)
	...
	private void DoPaniniProjection(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		PostProcessPassDataBase passData2;
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<PostProcessPassDataBase>("Panini Projection", out passData2);
		passData2.Source = renderGraphBuilder.ReadTexture(in source);
		passData2.Destination = renderGraphBuilder.WriteTexture(in destination);
		passData2.Materials = m_Materials;
		renderGraphBuilder.SetRenderFunc(delegate(PostProcessPassDataBase passData, RenderGraphContext context)
	...
	private void SetupBloom(ref RenderingData renderingData, TextureHandle source, Material uberMaterial)
	{
		...
		using (RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<BloomPassData>("Bloom", out passData2))
		{
			passData2.Source = renderGraphBuilder.ReadTexture(in source);
			passData2.Materials = m_Materials;
	...
	private void SetupBloomEnhanced(ref RenderingData renderingData, TextureHandle source, Material uberMaterial)
	{
		...
		using (RenderGraphBuilder renderGraphBuilder = renderGraph.AddRenderPass<BloomEnhancedPassData>("Bloom Enhanced", out passData2))
		{
			passData2.Source = renderGraphBuilder.ReadTexture(in source);
			passData2.Materials = m_Materials;
			passData2.BloomMipDown = m_BloomMipDown;
	...
	private void DoUberPass(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<UberPassData>("Uber Pass", out passData2);
		passData2.Source = renderGraphBuilder.ReadTexture(in source);
		passData2.Destination = renderGraphBuilder.WriteTexture(in destination);
		passData2.Materials = m_Materials;
		passData2.ViewMatrixToRestore = renderingData.CameraData.GetViewMatrix();
	...
	private void DoDaltonization(ref RenderingData renderingData, TextureHandle source, TextureHandle destination)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<DaltonizationPassData>("Daltonization", out passData2);
		passData2.Source = renderGraphBuilder.ReadTexture(in source);
		passData2.Destination = renderGraphBuilder.WriteTexture(in destination);
		passData2.Materials = m_Materials;
	...
	private void DoRcas(ref RenderingData renderingData, TextureHandle source, TextureHandle destination, float sharpness)
	{
		...
		using RenderGraphBuilder renderGraphBuilder = renderingData.RenderGraph.AddRenderPass<RcasPassData>("Rcas", out passData);
