...
public class UICornerCut : UIPrimitiveBase
{
	...
	protected override void OnPopulateMesh(VertexHelper vh)
	{
		...
		if (makeColumns)
		{
			vector = new Vector2(rect.xMin, cutUL ? rect2.yMax : rect.yMax);
			vector2 = new Vector2(rect.xMax, cutUR ? rect2.yMax : rect.yMax);
			vector3 = new Vector2(rect.xMin, cutLL ? rect2.yMin : rect.yMin);
			vector4 = new Vector2(rect.xMax, cutLR ? rect2.yMin : rect.yMin);
			if (flag3)
			{
				AddSquare(vector3, vector, new Vector2(rect2.xMin, rect.yMax), new Vector2(rect2.xMin, rect.yMin), rect, useColorUp ? colorUp : color, vh);
			}
			if (flag4)
			{
				AddSquare(vector2, vector4, new Vector2(rect2.xMax, rect.yMin), new Vector2(rect2.xMax, rect.yMax), rect, useColorDown ? colorDown : color, vh);
			}
		}
		else
		{
			vector = new Vector2(cutUL ? rect2.xMin : rect.xMin, rect.yMax);
			vector2 = new Vector2(cutUR ? rect2.xMax : rect.xMax, rect.yMax);
			vector3 = new Vector2(cutLL ? rect2.xMin : rect.xMin, rect.yMin);
			vector4 = new Vector2(cutLR ? rect2.xMax : rect.xMax, rect.yMin);
			if (flag2)
			{
				AddSquare(vector4, vector3, new Vector2(rect.xMin, rect2.yMin), new Vector2(rect.xMax, rect2.yMin), rect, useColorDown ? colorDown : color, vh);
			}
			if (flag)
			{
				AddSquare(vector, vector2, new Vector2(rect.xMax, rect2.yMax), new Vector2(rect.xMin, rect2.yMax), rect, useColorUp ? colorUp : color, vh);
			}
		}
		if (makeColumns)
		{
