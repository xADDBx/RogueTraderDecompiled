...
public class GreedyWidgetList : MonoBehaviour
{
	[SerializeField]
	[UsedImplicitly]
	...
	private IDisposable UpdateWidgets<TWidget>(IEnumerable<IViewModel> vmCollection) where TWidget : IWidgetView
	{
		...
		return Disposable.Create(HideAll);
	}

	private void SetEntityActive<TWidget>(TWidget entity, bool active) where TWidget : IWidgetView
	{
		if (entity is IHasVisibility hasVisibility)
		{
			hasVisibility.SetVisible(active);
		}
		else
		{
			entity.MonoBehaviour.gameObject.SetActive(active);
		}
	}
	...
	public void DrawEmptyEntities<TWidget>(TWidget prefab, int number) where TWidget : IWidgetView
	{
		...
		for (int i = m_CurrentCollectionLength; i < m_CurrentCollectionLength + number; i++)
		{
			...
			SetEntityActive(widgetView, active: true);
	...
	public void HideAll()
	{
		...
		{
			SetEntityActive(entry, active: false);
		}
	}
	...
	public void HideNotUsed(int currentCollectionLength)
	{
		if (m_Entries != null)
		{
			...
			{
				IWidgetView entity = m_Entries[i];
				SetEntityActive(entity, active: false);
			}
		}
	}
