...
public sealed class UnitDoNothingParams : UnitCommandParams<UnitDoNothing>, IMemoryPackable<UnitDoNothingParams>, IMemoryPackFormatterRegister
{
	...
	public static void Deserialize(ref MemoryPackReader reader, ref UnitDoNothingParams? value)
	{
		...
		if (memberCount == 20)
		{
			...
			reader.DangerousReadUnmanaged<WalkSpeedType?, bool?, bool?, float>(out value18, out value19, out value20, out value21);
		}
		else
		{
			if (memberCount > 20)
			{
				MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(UnitDoNothingParams), 20, memberCount);
				return;
			}
			if (value == null)
			{
