...
public sealed class PlayerUseAbilityParams : UnitUseAbilityParams, IMemoryPackable<PlayerUseAbilityParams>, IMemoryPackFormatterRegister
{
	...
	public static void Deserialize(ref MemoryPackReader reader, ref PlayerUseAbilityParams? value)
	{
		...
		bool value22;
		UnitPartStrategistKeystoneRearAbilityFlipPattern.FlipStates value23;
		string abilityId;
		string variantId;
		if (memberCount == 24)
		{
			if (value != null)
			{
				...
				reader.ReadUnmanaged<UnitPartStrategistKeystoneRearAbilityFlipPattern.FlipStates>(out value23);
				goto IL_04f9;
			}
			reader.ReadUnmanaged<CommandType>(out value2);
		...
		else
		{
			if (memberCount > 24)
			{
				MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(PlayerUseAbilityParams), 24, memberCount);
				return;
			}
			if (value == null)
			{
				...
				abilityId = null;
				variantId = null;
				value23 = UnitPartStrategistKeystoneRearAbilityFlipPattern.FlipStates.FlipNone;
			}
			else
			{
				value2 = value.Type;
