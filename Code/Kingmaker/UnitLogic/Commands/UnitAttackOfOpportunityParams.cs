	...
	public static void Deserialize(ref MemoryPackReader reader, ref UnitAttackOfOpportunityParams? value)
	{
		...
		if (memberCount == 20)
		{
			if (value == null)
			{
				...
				value21 = reader.ReadValue<BlueprintFact>();
			}
			else
			{
				value2 = value.Type;
		...
		else
		{
			if (memberCount > 20)
			{
				MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(UnitAttackOfOpportunityParams), 20, memberCount);
				return;
			}
			if (value == null)
			{
		...
		value = new UnitAttackOfOpportunityParams(value21)
		{
			Type = value2,
			OwnerRef = value3,
			m_Handle = value4,
