...
public sealed class UnitLootUnitParams : UnitCommandParams<UnitLootUnit>, IMemoryPackable<UnitLootUnitParams>, IMemoryPackFormatterRegister
{
	...
	public static void Serialize(ref MemoryPackWriter writer, ref UnitLootUnitParams? value)
	{
		...
		Vector3? value11 = value.OverrideApproachPoint;
		writer.DangerousWriteUnmanaged(in movementType, in isOneFrameCommand, in slowMotionRequired, in value11);
	}
	...
	public static void Deserialize(ref MemoryPackReader reader, ref UnitLootUnitParams? value)
	{
		...
		if (memberCount == 20)
		{
			if (value != null)
			{
				...
				value21 = value.OverrideApproachPoint;
				...
				reader.DangerousReadUnmanaged<Vector3?>(out value21);
				goto IL_042b;
			}
			reader.ReadUnmanaged<CommandType>(out value2);
			...
			reader.DangerousReadUnmanaged<WalkSpeedType?, bool?, bool?, Vector3?>(out value18, out value19, out value20, out value21);
		}
		else
		{
			if (memberCount > 20)
			{
				MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(UnitLootUnitParams), 20, memberCount);
				return;
			}
			if (value == null)
			{
		...
		value.OverrideApproachPoint = value21;
	}
}
