...
public sealed class UnitInteractWithUnitParams : UnitCommandParams<UnitInteractWithUnit>, IMemoryPackable<UnitInteractWithUnitParams>, IMemoryPackFormatterRegister
{
	[Preserve]
	private sealed class UnitInteractWithUnitParamsFormatter : MemoryPackFormatter<UnitInteractWithUnitParams>
	{
		[Preserve]
		public override void Serialize(ref MemoryPackWriter writer, ref UnitInteractWithUnitParams value)
		{
			UnitInteractWithUnitParams.Serialize(ref writer, ref value);
		}

		[Preserve]
		public override void Deserialize(ref MemoryPackReader reader, ref UnitInteractWithUnitParams value)
		{
			UnitInteractWithUnitParams.Deserialize(ref reader, ref value);
		}
	}

	[JsonProperty]
	[MemoryPackInclude]
	public EntityRef<AbstractUnitEntity> TargetUnit { get; private set; }

	[JsonConstructor]
	[MemoryPackConstructor]
	private UnitInteractWithUnitParams()
		: base(default(JsonConstructorMark))
	{
	}

	public UnitInteractWithUnitParams([CanBeNull] AbstractUnitEntity target)
		: base((target != null && target.IsExtra) ? ((TargetWrapper)target.Position) : ((TargetWrapper)target))
	{
		TargetUnit = target;
	}

	static UnitInteractWithUnitParams()
	{
		RegisterFormatter();
	}
	...
	{
		if (!MemoryPackFormatterProvider.IsRegistered<UnitInteractWithUnitParams>())
		{
			MemoryPackFormatterProvider.Register(new UnitInteractWithUnitParamsFormatter());
		}
		if (!MemoryPackFormatterProvider.IsRegistered<UnitInteractWithUnitParams[]>())
		{
			MemoryPackFormatterProvider.Register(new ArrayFormatter<UnitInteractWithUnitParams>());
		}
	...
	[Preserve]
	public static void Serialize(ref MemoryPackWriter writer, ref UnitInteractWithUnitParams? value)
	{
		if (value == null)
		{
		...
		EntityRef<AbstractUnitEntity> value11 = value.TargetUnit;
		writer.WritePackable(in value11);
	}

	[Preserve]
	public static void Deserialize(ref MemoryPackReader reader, ref UnitInteractWithUnitParams? value)
	{
		...
		bool? value19;
		bool? value20;
		EntityRef<AbstractUnitEntity> value21;
		if (memberCount == 20)
		{
			if (value != null)
			{
				...
				reader.ReadPackable(ref value21);
				goto IL_0431;
			}
			reader.ReadUnmanaged<CommandType>(out value2);
			...
			value21 = reader.ReadPackable<EntityRef<AbstractUnitEntity>>();
		}
		else
		{
			if (memberCount > 20)
			{
				MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(UnitInteractWithUnitParams), 20, memberCount);
				return;
			}
			if (value == null)
			{
				...
				value19 = null;
				value20 = null;
				value21 = default(EntityRef<AbstractUnitEntity>);
			}
			else
			{
				value2 = value.Type;
		...
		value = new UnitInteractWithUnitParams
		{
			Type = value2,
			OwnerRef = value3,
			m_Handle = value4,
