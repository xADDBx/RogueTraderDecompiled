...
[TypeId("5930714e6c92d6a4c9e01295a4101d20")]
public class AddFactsToPet : UnitFactComponentDelegate, IDifficultyChangedClassHandler, ISubscriber, IHashable
{
	[NotNull]
	[SerializeField]
	...
	{
		BaseUnitEntity pet = base.Owner.Parts.GetOptional<UnitPartPetOwner>()?.PetUnit;
		if (pet == null)
		{
			return;
		}
		...
		{
			EntityFact[] array = pet.Facts.FindAllBySource(base.Fact, this).ToArray();
			for (int i = 0; i < array.Length; i++)
			{
				array[i]?.Reapply();
			}
			EventBus.RaiseEvent((IBaseUnitEntity)base.Owner, (Action<IPetSetupHandle>)delegate(IPetSetupHandle h)
			{
				h.HandlePetSetup(pet);
			}, isCheckRuntime: true);
		}
		else
		{
	...
	{
		BaseUnitEntity baseUnitEntity = base.Owner.Parts.GetOptional<UnitPartPetOwner>()?.PetUnit;
		if (baseUnitEntity != null)
		{
			EntityFact[] array = baseUnitEntity.Facts.FindAllBySource(base.Fact, this).ToArray();
			for (int i = 0; i < array.Length; i++)
			{
				array[i]?.Reapply();
			}
	...
	public void HandleDifficultyChanged()
	{
		...
		{
			if (baseUnitEntity.Facts.FindBySource(fact, base.Fact, this) == null)
			{
				list.Add(fact);
			}
		}
	...
	private void UpdateFacts(bool postLoad)
	{
		...
		foreach (EntityFact item in baseUnitEntity.Facts.FindAllBySource(base.Fact, this))
		{
		...
		{
			baseUnitEntity.Facts.Remove(item2);
		}
	...
	private void TryAddFact(BlueprintUnitFact blueprintFact)
	{
		...
		if (baseUnitEntity != null)
		{
			MechanicEntityFact fact = blueprintFact.CreateFact(null, baseUnitEntity, null);
			baseUnitEntity.Facts.Add(fact)?.AddSource(base.Fact, this);
		}
	}
	...
	private void Clear()
	{
		...
		if (baseUnitEntity != null)
		{
			RemoveAllFactsOriginatedFromThisComponent(baseUnitEntity);
		}
	}
