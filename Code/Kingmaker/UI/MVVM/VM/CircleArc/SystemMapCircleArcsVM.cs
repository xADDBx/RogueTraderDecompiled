...
public class SystemMapCircleArcsVM : BaseCircleArcsVM, IStarSystemShipMovementHandler, ISubscriber
{
	...
	{
		IsCorrectGameMode.Value = gameMode == GameModeType.StarSystem || (gameMode == GameModeType.Dialog && m_PreviousMode == GameModeType.StarSystem);
	}

	protected override void OnGameModeStopImpl(GameModeType gameMode)
	{
		IsCorrectGameMode.Value = Game.Instance.CurrentMode == GameModeType.StarSystem || (Game.Instance.CurrentMode == GameModeType.Dialog && m_PreviousMode == GameModeType.StarSystem);
	}
