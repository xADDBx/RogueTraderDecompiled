...
public class ExplorationSpaceEventBaseView<DialogAnswer, DialogSystem> : ViewBase<SpaceEventVM>, IGameModeHandler, ISubscriber where DialogAnswer : DialogAnswerBaseView where DialogSystem : DialogSystemAnswerBaseView
{
	[Header("Body Block")]
	[SerializeField]
	...
	protected override void BindViewImplementation()
	{
		...
		AddDisposable(base.ViewModel.History.ObserveAdd().Subscribe(delegate(CollectionAddEvent<IDialogShowData> value)
		{
			...
			m_HistoryContainer.gameObject.SetActive(value: true);
	...
	protected void BindSystemAnswer(AnswerVM answerVM)
	{
		if (answerVM != null)
		{
	...
	protected void CreateInputImpl(InputLayer inputLayer)
	{
	...
	private void SetVisible(bool state, bool force = false, UnityAction onCompleteAction = null)
	{
		if (m_VisibleState != state)
		{
			m_VisibleState = state;
			m_Animator.PlayAnimation(state, onCompleteAction);
		}
	}
