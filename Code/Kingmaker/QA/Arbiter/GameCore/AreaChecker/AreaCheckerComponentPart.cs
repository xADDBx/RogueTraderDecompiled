...
public class AreaCheckerComponentPart : ArbiterElement
{
	...
	public ArbiterElementList PointList = new ArbiterElementList(typeof(ArbiterPointElement));
	...
	public string AreaPartLightScenes => string.Join(", ", from x in ArbiterIntegration.GetLightScenes(EnterPoint.Get())
		select x.SceneName);

	public IEnumerable<ArbiterPointElement> Points => PointList.ElementList.Select((ArbiterElement x) => x as ArbiterPointElement);
