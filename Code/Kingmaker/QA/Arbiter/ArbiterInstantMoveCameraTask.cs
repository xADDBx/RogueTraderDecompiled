...
public class ArbiterInstantMoveCameraTask : ArbiterTask
{
	...
	protected override IEnumerator Routine()
	{
		...
		BlueprintAreaReference blueprintAreaReference = (ArbiterInstructionIndex.Instance.GetInstruction(m_BlueprintAreaName)?.Test as AreaCheckerComponent)?.Area;
		...
		{
			PFLog.Arbiter.Log("Loading " + areaPart?.NameSafe());
			base.Status = "Loading";
		...
		{
			ArbiterClientIntegration.SetGamePause(value: true);
			...
			yield return 3f;
			ArbiterClientIntegration.SetGamePause(value: true);
			ArbiterClientIntegration.MoveCameraImmediately(m_Position, m_Rotation, m_Zoom);
		}
	}
}
