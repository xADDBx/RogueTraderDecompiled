...
public class ArbiterClientMeasurements
{
	...
	[Measurement("FPS")]
	public static int FPS => Mathf.RoundToInt(1f / DeltaTimeCounter.GetAvgValue(95));

	[Measurement("AwakeUnits")]
	public static int AwakeUnits => Game.Instance.State.AllAwakeUnits.Count;

	[Measurement("TotalUnits")]
	public static int TotalUnits => Game.Instance.State.AllUnits.All.Count;

	[Measurement("UniqueUnitPrefabs")]
	public static int UniqueUnitPrefabs => Game.Instance.State.AllUnits.All.Select((AbstractUnitEntity u) => u.ViewSettings.PrefabGuid).Distinct().Count();

	[Measurement("Corpses")]
	public static int Corpses => Game.Instance.State.AllUnits.All.Count((AbstractUnitEntity u) => u.LifeState.IsDead);

	[Measurement("MapObjects")]
	public static int MapObjects => Game.Instance.State.MapObjects.Count();

	[Measurement("InteractiveObjects")]
	public static int InteractiveObjects => Game.Instance.State.MapObjects.Count((MapObjectEntity m) => m.View.GetComponent<IInteractionComponent>() != null);

	[Measurement("Traps")]
	public static int Traps => Game.Instance.State.MapObjects.OfType<TrapObjectData>().Count();

	[Measurement("Cutscenes")]
	public static int Cutscenes => Game.Instance.State.Cutscenes.Count();

	[Measurement("DeltaTime")]
	public static float DeltaTime => DeltaTimeCounter.GetAvgValue(95);

	[Measurement("Memory.Allocated")]
	public static int AllocatedMemory => (int)(Profiler.GetTotalAllocatedMemoryLong() / s_MemoryFactor);

	[Measurement("Memory.Reserved")]
	public static int ReservedMemory => (int)(Profiler.GetTotalReservedMemoryLong() / s_MemoryFactor);

	[Measurement("Memory.Total")]
	...
	[Measurement("Memory.SystemUsed")]
	public static int SystemUsedMemory
	{
		get
		{
			if (!s_SystemUsedMemoryRecorder.Valid)
	...
	[Measurement("Memory.TotalUsed")]
	public static int TotalUsedMemory
	{
		get
		{
	...
	[Measurement("Memory.TotalReserved")]
	...
	[Measurement("Memory.GCUsed")]
	public static int GCUsedMemory
	{
		get
		{
	...
	[Measurement("Memory.GCReserved")]
	public static int GCReservedMemory
	{
		get
		{
	...
	[Measurement("Memory.ProfilerUsed")]
	public static int ProfilerUsedMemory
	{
		get
		{
	...
	[Measurement("Memory.ProfilerReserved")]
	public static int ProfilerReservedMemory
	{
		get
		{
	...
	[Measurement("Memory.AudioUsed")]
	public static int AudioUsedMemory
	{
		get
		{
	...
	[Measurement("Memory.AudioReserved")]
	public static int AudioReservedMemory
	{
		get
		{
	...
	[Measurement("Memory.VideoUsed")]
	public static int VideoUsedMemory
	{
		get
		{
	...
	[Measurement("Memory.VideoReserved")]
	public static int VideoReservedMemory
	{
		get
		{
	...
	[Measurement("RenderTexturesCount")]
	public static int RenderTexturesCount
	{
		get
		{
	...
	[Measurement("RenderTexturesBytes")]
	public static int RenderTexturesBytes
	{
		get
		{
	...
	[Measurement("MonoUsedSize")]
	public static int MonoUsedSize => (int)(Profiler.GetMonoUsedSizeLong() / s_MemoryFactor);

	[Measurement("MonoHeapSize")]
	public static int MonoHeapSize => (int)(Profiler.GetMonoHeapSizeLong() / s_MemoryFactor);

	[Measurement("Memory.Gpu")]
	public static int GpuMemory => (int)(Profiler.GetAllocatedMemoryForGraphicsDriver() / s_MemoryFactor);
	...
	public static Dictionary<string, string> GetAreaPointMeasurements()
	{
		...
		foreach (PropertyInfo item in from prop in typeof(ArbiterClientMeasurements).GetProperties(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
			where Attribute.IsDefined(prop, typeof(MeasurementAttribute))
			select prop)
		{
			string name = ((MeasurementAttribute)Attribute.GetCustomAttribute(item, typeof(MeasurementAttribute))).Name;
			try
			{
	...
	{
		Dictionary<string, string> dictionary = new Dictionary<string, string>();
		foreach (PropertyInfo item in from p in typeof(ArbiterClientMeasurements).GetProperties(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic).ToList()
	...
	{
		return (from p in typeof(ArbiterClientMeasurements).GetProperties(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
			where p.PropertyType == typeof(MeasurementCounter)
