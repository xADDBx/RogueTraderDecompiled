...
public class AugmentationsInventoryStashView : ViewBase<AugmentationsInventoryStashVM>
{
	[SerializeField]
	...
	[FormerlySerializedAs("ItemsFilter")]
	[SerializeField]
	protected AugmentationsFiltersPCView m_ItemsFilter;
	...
	protected override void BindViewImplementation()
	{
		...
		AddDisposable(base.ViewModel.UpdateMetallizationBuffTooltip.Subscribe(UpdateTooltipHandler));
	...
	public void UpdateTooltipHandler()
	{
		Buff buff = base.ViewModel.Unit.Value.Buffs.GetBuff(BlueprintRoot.Instance.SystemMechanics.MetallicizationBuff);
		if (buff == null)
		{
			buff = BlueprintRoot.Instance.SystemMechanics.MetallicizationBuff.CreateFact(null, base.ViewModel.Unit.Value, default(BuffDuration)) as Buff;
			buff?.SetRankToZero();
		}
		TooltipTemplateBuff template = new TooltipTemplateBuff(buff);
		AddDisposable(m_CoinsContainer.SetTooltip(template));
	}
}
