...
public class EGSAchievementsManager : IPlatformAchievementHandler, IDisposable
{
	...
	public async void SyncAchievements()
	{
		...
		foreach (AchievementEntity achievement in m_Achievements)
		{
			...
			else if (achievement.IsUnlocked && !eOSPlayerAchievmentData.Unlocked)
			{
			...
			{
				m_AchievementsHelper.AddStatToUpdateList(eGSId, achievement.Counter - eOSPlayerAchievmentData.StatCurrentValue);
			}
		}
		m_IsSynced = true;
	...
	public void OnAchievementProgressUpdated(AchievementEntity achievementEntity)
	{
		if (m_IsSynced && m_IsEGSAchievementsHelperReady)
		{
			Logger.Log($"AddStatToUpdateList: {achievementEntity} ({achievementEntity.Data.EGSId})");
			m_AchievementsHelper.AddStatToUpdateList(achievementEntity.Data.EGSId, 1);
		}
	}
}
