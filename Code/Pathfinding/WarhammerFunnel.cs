...
public class WarhammerFunnel
{
	...
	public static FunnelPortals ConstructFunnelPortals(List<GraphNode> nodes, PathPart part)
	{
		...
		{
			if (nodes[i] is CustomGridNode && i > part.startIndex)
			{
				Vector2Int coordinatesInGrid = ((CustomGridNode)nodes[i - 1]).CoordinatesInGrid;
				Vector2Int coordinatesInGrid2 = ((CustomGridNode)nodes[i]).CoordinatesInGrid;
				Vector2Int coordinatesInGrid3 = ((CustomGridNode)nodes[i + 1]).CoordinatesInGrid;
				...
				{
					CustomGridNode.DiagonalPortalInfo? diagonalPortalRich = ((CustomGridNode)nodes[i - 1]).GetDiagonalPortalRich(nodes[i + 1], checkHasConnection: false);
					if (diagonalPortalRich.HasValue)
					{
						CustomGridNode.DiagonalPortalInfo valueOrDefault = diagonalPortalRich.GetValueOrDefault();
